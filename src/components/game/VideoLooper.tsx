...
export interface VideoLooperProps {
  /** Video URL (mp4/webm). Loops while dialogue advances. */
  videoSrc: string;
  /** Poster frame while video loads (optional) */
  posterSrc?: string;
  /** Mute by default (required for autoplay in most browsers) */
  muted?: boolean;
...
/**
 * VIDEO LOOPER — looping video background + multi-line dialogue.
 * Video keeps playing; only text advances on Continue.
 * Ends when the user continues past the last line.
 */
export default function VideoLooper({
  ...
  // Keep looping after remounts / visibility
  useEffect(() => {
    ...
    const play = () => {
      void el.play().catch(() => {
        /* autoplay may be blocked until user gesture — Continue will still work */
      });
    };
    play();
  ...
  if (script.length === 0) {
    return (
      ...
      >
        <p className="text-sm text-zinc-500">VideoLooper: no dialogue lines</p>
      </div>
    );
  }
